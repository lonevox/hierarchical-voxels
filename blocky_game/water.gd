...
@onready var _multi_terrain: VoxelMultiTerrain = get_node("../VoxelMultiTerrain")
...
func _process_cell(pos: Vector3):
	# TODO: Make this work for all terrains
	var voxel_tool := _voxel_tool.voxel_tools[_multi_terrain.terrains[0]]
	var v := voxel_tool.get_voxel(pos)
...
func _fill_with_water(pos: Vector3):
	...
	var above_v := voxel_tool.get_voxel(above)
	var below_v := voxel_tool.get_voxel(below)
	var above_rm := _blocks.get_raw_mapping(above_v)
	# Make sure the top has the surface model
	if above_rm.block_id == _water_id:
		voxel_tool.set_voxel(pos, _water_full)
	else:
		voxel_tool.set_voxel(pos, _water_top)
	if below_v == _water_top:
		voxel_tool.set_voxel(below, _water_full)
